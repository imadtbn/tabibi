...
def page(path,title,desc,body,active='',noindex=False,extra_schema=None):
    ...
    extra_scripts=f'<script type="module" src="{prefix}js/contact.js"></script>' if path=='contact.html' else ''
...
'contact.html':('التواصل وتصحيح البيانات','أرسل ملاحظات تقنية أو طلب تصحيح بيانات منشورة في دليل طبيبي.','''<section class="contribution-panel section" aria-labelledby="contact-form-title"><div><span class="eyebrow">رسالة مباشرة</span><h2 id="contact-form-title">أرسل رسالتك إلى فريق طبيبي</h2><p>تُحفظ الرسائل في ملف Google Sheets خاص بالمراجعة. لا ترسل معلومات صحية شخصية أو ملفات طبية.</p></div><form id="contact-form" class="editor-form"><label class="field">الاسم<input name="name" required maxlength="120" autocomplete="name" placeholder="اسمك"></label><label class="field">البريد الإلكتروني (اختياري)<input name="email" type="email" maxlength="160" autocomplete="email" placeholder="name@example.com"></label><label class="field">الموضوع<input name="subject" required maxlength="180" placeholder="موضوع الرسالة"></label><label class="field">الرسالة<textarea name="message" required minlength="10" maxlength="5000" rows="7" placeholder="اكتب رسالتك هنا"></textarea></label><label class="field hp-field" aria-hidden="true">اترك هذا الحقل فارغًا<input name="website" type="text" tabindex="-1" autocomplete="off"></label><button class="btn primary" type="submit">إرسال الرسالة</button><div id="contact-state" class="submission-state" role="status" aria-live="polite">تُرسل الرسالة إلى فريق المراجعة.</div></form><iframe name="contact-submit-frame" title="استقبال الرسالة" hidden></iframe></section><article class="prose section"><h2>ملاحظات التصحيح</h2><p>لطلب تصحيح بيانات طبيب، اذكر رابط الصفحة والمعلومة المطلوب تعديلها ومصدرًا علنيًا يؤكد التصحيح.</p><a class="btn secondary" href="https://github.com/imadtbn/tabibi/issues" target="_blank" rel="noopener noreferrer">فتح صفحة ملاحظات المشروع ↗</a><p class="notice">للاستفسار عن موعد أو علاج، اتصل بالعيادة مباشرة. هذه الصفحة لا ترسل رسائل إلى الأطباء.</p></article>'''),
