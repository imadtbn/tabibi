...
specs=load('data/specialties.json'); wilayas=load('data/wilayas.json'); doctors=load('data/doctors.json'); communes=load('data/communes.json')
...
def validate_records(records):
    ...
    for d in records:
        ...
        assert d.get('verificationStatus')=='verified','Only verified records may be published'
...
def spec_cards(prefix='',subset=None):
    return '<div class="specialties-grid">'+''.join(f'<a class="specialty-card" href="{prefix}specialties/{s["slug"]}.html"><span class="spec-symbol">{ICON}</span><h3>{esc(s["nameAr"])}</h3><span class="latin" lang="fr">{esc(s["nameFr"])}</span><span class="card-arrow" aria-hidden="true">↖</span></a>' for s in (subset or specs))+'</div>'
...
<label class="field"><span>التخصص الطبي</span><select name="spec" id="spec" {'disabled' if spec else ''}><option value="">جميع التخصصات</option>{options(specs,'id','nameAr')}</select></label>
...
def static_doctor(d,prefix=''):
    p=d['practices'][0]
    return f'<article class="doctor-card"><span class="badge">بيانات موثقة المصدر</span><h2><a href="{prefix}doctors/{d["slug"]}.html">{esc(d["name"])}</a></h2><p>{esc(" • ".join(spec_by[x]["nameAr"] for x in d["specialtyIds"]))}</p><p>{esc(wilaya_by[p["wilayaCode"]]["nameAr"])} — {esc(p["address"])}</p><small>آخر تحقق: {esc(d["verifiedAt"])}</small><a class="btn secondary" href="{prefix}doctors/{d["slug"]}.html">عرض التفاصيل</a></article>'
...
{filters(spec)}<div class="filter-tools"><label class="field compact">البلدية<select id="commune"><option value="">كل البلديات المتاحة</option></select></label><label class="field compact">الترتيب<select id="sort"><option value="name">حسب الاسم</option><option value="recent">آخر تحقق</option><option value="distance">الأقرب إليّ</option></select></label><button class="btn secondary" id="locate" type="button">⌖ الأقرب إليّ</button><button class="text-button" id="reset" type="button">مسح الفلاتر</button></div>
...
def page(path,title,desc,body,active='',noindex=False,extra_schema=None):
    ...
    nav=[('index.html','الرئيسية','home'),('specialties/index.html','التخصصات','specialties'),('doctors.html','الأطباء','doctors'),('map.html','الخريطة','map'),('favorites.html','المفضلة','favorites')]
...
<div class="trust-line"><span>معلومة من مصدر واضح</span><span>تاريخ تحقق معلن</span><span>لا حاجة إلى إنشاء حساب</span></div>
...
<section class="notice editorial"><h2>نبني دليلًا يستحق ثقتك</h2><p>الدليل في مرحلة جمع البيانات والتحقق منها. لا ننشر أسماء أو أرقام أطباء تجريبية. ستظهر السجلات عند توفر مصادر موثوقة.</p><a class="text-link" href="about.html">كيف نتحقق من المعلومات؟ ←</a></section>'''
...
page('stats.html','إحصائيات الدليل','أعداد السجلات المنشورة والتخصصات والولايات التي تتوفر فيها بيانات موثقة في دليل طبيبي.',heading('الدليل بالأرقام','هذه أعداد سجلات طبيبي المنشورة، وليست إحصاءً لجميع أطباء الجزائر.')+f'<section class="stats-grid section"><article><span>{len(doctors)}</span><h2>طبيب موثق المصدر</h2></article><article><span>{covered}</span><h2>ولاية بها سجلات</h2></article><article><span>{len(specs)}</span><h2>تخصصًا في الدليل</h2></article></section><p class="notice">تُحدّث الأرقام تلقائيًا مع كل إصدار للبيانات. لا تدخل السجلات التجريبية في الإحصائيات.</p>')
articles={
'about.html':('عن طبيبي والتحقق من البيانات','تعرف على هدف دليل طبيبي وكيفية قبول المعلومات وتحديثها.','''<h2>معلومات أوضح، واختيار أسهل</h2><p>طبيبي دليل يساعد المستخدم في الجزائر على العثور على معلومات الأطباء والعيادات حسب التخصص والمكان. لا يقدم تشخيصًا طبيًا أو حجز مواعيد مباشرًا.</p><h2>كيف تُنشر المعلومات؟</h2><ol><li>الحصول على المعلومات من مصدر علني موثوق أو من العيادة مباشرة.</li><li>التحقق من الاسم والتخصص والعنوان ووسائل الاتصال.</li><li>إرفاق مصدر وتاريخ تحقق بالسجل قبل نشره.</li><li>مراجعة التصحيحات وتحديث المعلومات عند ثبوت التغيير.</li></ol><h2>حالة الدليل</h2><p>بدأنا بتنظيم التخصصات وتجهيز البحث. لم تُنشر بعد سجلات أطباء موثقة. أُزيلت البيانات التجريبية من العرض العام.</p><p>وصف «موثق المصدر» يتعلق بمصدر بيانات الاتصال، ولا يمثل اعتمادًا طبيًا أو تقييمًا لكفاءة الطبيب.</p>'''),
...
for d in doctors:
    practices=''
    for p in d['practices']:
        calls=' '.join(f'<a class="btn primary" href="tel:{esc(n)}">اتصال <bdi>{esc(n)}</bdi></a>' for n in p.get('phones',[]))
        geo=p.get('coordinates');direction=f'<a class="btn secondary" rel="noopener noreferrer" target="_blank" href="https://www.google.com/maps/dir/?api=1&amp;destination={geo["lat"]},{geo["lng"]}">الاتجاهات ↗</a>' if geo else ''
        practices+=f'<section class="doctor-card"><h2>{esc(wilaya_by[p["wilayaCode"]]["nameAr"])}</h2><p>{esc(p["address"])}</p><div class="card-actions">{calls}{direction}</div><p>اتصل لتأكيد ساعات العمل والتوفر قبل الزيارة.</p></section>'
    body=heading(esc(d['name']),esc(' • '.join(spec_by[x]['nameAr'] for x in d['specialtyIds'])))+f'<div class="section">{practices}<p>آخر تحقق: <time>{esc(d["verifiedAt"])}</time></p><a class="text-link" href="{esc(d["sourceUrl"])}" rel="noopener noreferrer" target="_blank">مصدر المعلومات ↗</a></div>'
    practice=d['practices'][0]
    schema={'@context':'https://schema.org','@type':'Physician','name':d['name'],'url':BASE+f'doctors/{d["slug"]}.html','address':{'@type':'PostalAddress','streetAddress':practice['address'],'addressRegion':wilaya_by[practice['wilayaCode']]['nameAr'],'addressCountry':'DZ'}}
    if practice.get('phones'):schema['telephone']=practice['phones'][0]
    if practice.get('coordinates'):schema['geo']={'@type':'GeoCoordinates','latitude':practice['coordinates']['lat'],'longitude':practice['coordinates']['lng']}
    page(f'doctors/{d["slug"]}.html',d['name'],f'عنوان ووسائل اتصال {d["name"]} ومصدر المعلومات وتاريخ التحقق.',body,'doctors',extra_schema=schema)
dump('data/stats.json',{'doctors':len(doctors),'coveredWilayas':covered,'specialties':len(specs)})
def norm(t):return re.sub('[\u064b-\u065f\u0670ـ]','',t).translate(str.maketrans('أإآى','اااي')).casefold()
index=[]
for d in doctors:
    terms=[d['name']]+[term for sid in d['specialtyIds'] for term in [spec_by[sid]['nameAr'],spec_by[sid]['nameFr'],*spec_by[sid]['aliases']]]+[term for p in d['practices'] for term in [p['address'],wilaya_by[p['wilayaCode']]['nameAr'],commune_by.get(p.get('communeId'),{}).get('nameAr','')]]
    index.append({'id':d['id'],'text':norm(' '.join(terms))})
...
(R/'sw.js').write_text(worker_template.replace('__VERSION__',version).replace('__PRECACHE__',json.dumps(assets,ensure_ascii=False)))
(R/'.nojekyll').touch()
print(f'Built {len(pages)} pages; {len(urls)} sitemap URLs; {len(doctors)} verified doctors; cache {version}')
