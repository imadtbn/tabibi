...
<main id="main" class="container">{'<p class="notice demo-banner">الدليل قيد استكمال البيانات: السجلات الأولية غير متحقق منها، والسجلات المسماة «نموذج» ليست أطباء حقيقيين. تحقق من المعلومات قبل الاتصال أو التنقل.</p>' if initial_doctors else ''}{body}</main>
...
page('add-doctor.html','إضافة وتعديل بيانات طبيب','أدخل بيانات الأطباء حسب التخصص، واحفظ مسوداتك محليًا ثم صدّر ملف التخصص للنشر.',heading('أثرِ الدليل بمعلومات طبيب','إضافة بيانات جديدة، أو تعديل سجل موجود، أو تصدير ملف تخصص كامل.')+editor,'add',True)
