...
specs=load('data/specialties.json'); wilayas=load('data/wilayas.json'); verified_doctors=load('data/doctors.json'); demo_doctors=load('data/demo-doctors.json'); doctors=verified_doctors+demo_doctors; communes=load('data/communes.json')
...
def validate_records(records):
    ...
    for d in records:
        ...
        if d.get('isDemo') is True:
            assert d.get('verificationStatus')=='demo' and d['id'].startswith('demo-') and d['slug'].startswith('demo-'),'Demo markers required'
            assert not d.get('sourceUrl') and not d.get('verifiedAt'),'Demo must not claim verification'
        else:
            assert d.get('verificationStatus')=='verified','Only verified real records may be published'
...
assert all(d.get('isDemo') is True for d in demo_doctors),'Separate demo dataset required'
assert all(not d.get('isDemo') for d in verified_doctors),'Real dataset must not contain demos'
...
def static_doctor(d,prefix='',practice=None):
    p=practice or d['practices'][0];days,times=schedule(p);demo=d.get('isDemo',False)
    status='نموذج وهمي · للتجربة فقط' if demo else 'بيانات موثقة المصدر'
    address='، '.join([wilaya_by[p['wilayaCode']]['nameAr'],commune_by.get(p.get('communeId'),{}).get('nameAr',''),p['address']])
...
<span class="availability unknown" data-hours="{esc(json.dumps(p.get('openingHours')))}">التوفر حسب توقيت الجزائر</span><p class="doctor-address">⌖ {esc(address)}</p><dl class="doctor-schedule"><div><dt>أيام العمل</dt><dd>{esc(days)}</dd></div><div><dt>توقيت العمل</dt><dd><bdi dir="ltr">{esc(times)}</bdi></dd></div></dl>{phones}{'<small class="demo-note">الهاتف غير حقيقي، والمسار إلى نقطة تجريبية بمركز المدينة.</small>' if demo else '<small>آخر تحقق: '+esc(d['verifiedAt'])+'</small>'}<div class="card-actions">{route}</div></article>'''
def statistics():
    counts=[('total',len(doctors),'سجل طبيب'),('specialties',len({x for d in doctors for x in d['specialtyIds']}),'تخصص به أطباء'),('wilayas',len({p['wilayaCode'] for d in doctors for p in d['practices']}),'ولاية في العرض'),('open','—','متوفر الآن حسب الجدول'),('demo',len(demo_doctors),'سجل تجريبي'),('verified',len(verified_doctors),'سجل حقيقي موثق')]
    return '<section class="stats-grid section" aria-label="إحصائيات عامة">'+''.join(f'<article><span data-stat="{key}">{value}</span><h2>{label}</h2></article>' for key,value,label in counts)+'</section><p class="muted">تشمل الإحصائيات البيانات التجريبية المعلّمة. يُحدّث التوفر كل 30 ثانية بتوقيت الجزائر.</p>'
...
def page(path,title,desc,body,active='',noindex=False,extra_schema=None):
    ...
    if demo_doctors and (path in ['doctors.html','map.html','stats.html'] or (path.startswith('specialties/') and path!='specialties/index.html')): noindex=True
...
<main id="main" class="container">{'<p class="notice demo-banner">وضع العرض التجريبي: أسماء الأطباء والهواتف وعناوين الشوارع وهمية. المواقع نقاط بمراكز المدن لا تمثل عيادات، والتوفر محاكاة للجدول.</p>' if demo_doctors else ''}{body}</main>
...
<div class="trust-line"><span>بيانات تجريبية واضحة</span><span>تحديث تلقائي للتوفر</span><span>لا حاجة إلى إنشاء حساب</span></div>{statistics()}
...
<section class="notice editorial"><h2>نبني دليلًا يستحق ثقتك</h2><p>يمكنك الآن تجربة الدليل بطبيبين وهميين لكل تخصص. تظل السجلات التجريبية مميزة بوضوح إلى أن تُستبدل ببيانات موثقة.</p><a class="text-link" href="about.html">كيف نتحقق من المعلومات؟ ←</a></section>'''
...
page('stats.html','إحصائيات الدليل','إحصائيات الأطباء والتخصصات والولايات، مع فصل الأعداد التجريبية عن الموثقة.',heading('الدليل بالأرقام','عدادات من البيانات المعروضة؛ لا تمثل إحصاءً طبيًا رسميًا.')+statistics())
articles={
'about.html':('عن طبيبي والتحقق من البيانات','تعرف على هدف دليل طبيبي وكيفية قبول المعلومات وتحديثها.','''<h2>معلومات أوضح، واختيار أسهل</h2><p>طبيبي دليل يساعد المستخدم في الجزائر على العثور على معلومات الأطباء والعيادات حسب التخصص والمكان. لا يقدم تشخيصًا طبيًا أو حجز مواعيد مباشرًا.</p><h2>كيف تُنشر المعلومات؟</h2><ol><li>الحصول على المعلومات من مصدر علني موثوق أو من العيادة مباشرة.</li><li>التحقق من الاسم والتخصص والعنوان ووسائل الاتصال.</li><li>إرفاق مصدر وتاريخ تحقق بالسجل قبل نشره.</li><li>مراجعة التصحيحات وتحديث المعلومات عند ثبوت التغيير.</li></ol><h2>حالة الدليل</h2><p>بدأنا بتنظيم التخصصات وتجهيز البحث. توجد نماذج وهمية لاختبار تجربة الاستخدام: طبيبان لكل تخصص. تحمل النماذج شارة واضحة ولا تُعرض كأطباء موثقين.</p><p>وصف «موثق المصدر» يتعلق بمصدر بيانات الاتصال، ولا يمثل اعتمادًا طبيًا أو تقييمًا لكفاءة الطبيب.</p>'''),
...
for d in doctors:
    ...
    page(f'doctors/{d["slug"]}.html',d['name'],'نموذج طبيب وهمي لتجربة تصميم البطاقة والتوفر حسب الجدول.' if demo else f'عنوان ووسائل اتصال {d["name"]}',body,'doctors',noindex=demo,extra_schema=schema)
dump('data/stats.json',{'doctors':len(doctors),'coveredWilayas':covered,'specialties':len(specs),'demoDoctors':len(demo_doctors),'verifiedDoctors':len(verified_doctors),'bySpecialty':{s['id']:sum(s['id'] in d['specialtyIds'] for d in doctors) for s in specs}})
...
(R/'.nojekyll').touch()
print(f'Built {len(pages)} pages; {len(urls)} sitemap URLs; {len(doctors)} doctors ({len(demo_doctors)} demo); cache {version}')
