import {url,loadData,el,favorites,storage,toast,locate,selectDoctors,scheduleText} from './core.js';
...
export async function initDirectory(){
  ...
  function card({doctor,practice,km}){
    const article=el('article',undefined,'doctor-card');article.append(el('span',doctor.isDemo?'نموذج وهمي · للتجربة فقط':'بيانات موثقة المصدر',doctor.isDemo?'badge demo-badge':'badge'));
    ...
    const commune=data.communes.find(c=>c.id===practice.communeId)?.nameAr;
    ...
    article.append(el('small',doctor.isDemo?'الهاتف غير حقيقي، والمسار إلى نقطة تجريبية بمركز المدينة.':`آخر تحقق: ${doctor.verifiedAt}`,doctor.isDemo?'demo-note':''));
  ...
  function render(){if(!data)return;const rows=selectDoctors(data,filters(),userLocation,savedOnly?favorites():null);nodes.results.replaceChildren(...rows.slice(0,limit).map(card));nodes['result-count'].textContent=`${rows.length} نتيجة`;nodes.empty.hidden=rows.length>0;nodes['load-more'].hidden=rows.length<=limit;updateAvailability();
