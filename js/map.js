...
export function initMap(){
  ...
  async function render(){
    ...
    for(const {doctor} of rows)for(const practice of doctor.practices){
      ...
      const popup=el('div',undefined,'map-popup');popup.append(el('strong',doctor.name),el('p',practice.communeName||''));if(doctor.verificationStatus!=='verified')popup.append(el('p','بيانات أولية؛ نقاط النماذج لا تمثل عيادات حقيقية')); const link=el('a','تفاصيل الطبيب');link.href=url(`doctors/${doctor.slug}.html`);popup.append(link);
