import {loadData,availability,summarize} from './core.js';
...
export function initMetrics(){
  ...
  function refresh(){
    ...
    const stats=summarize(data,now,navigator.onLine);
    for(const node of document.querySelectorAll('[data-specialty-count]')){const count=stats.bySpecialty[node.dataset.specialtyCount]||0;node.querySelector('b').textContent=String(count).padStart(2,'0');node.setAttribute('aria-label',`${count} طبيب في هذا التخصص، بما فيها النماذج التجريبية`);}
  ...
  if(document.querySelector('[data-stat],[data-specialty-count]'))loadData().then(result=>{data=result;refresh();}).catch(()=>{});
